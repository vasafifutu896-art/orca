...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('$cwdUriObject = [System.Uri]::new($location.ProviderPath)')
    expect(script).toContain('if ($cwdUriObject.IsUnc)')
    expect(script).toContain('file:////$($cwdUriObject.Host)$($cwdUriObject.AbsolutePath)')
    expect(script).toContain(')]7;$cwdUri$(')
